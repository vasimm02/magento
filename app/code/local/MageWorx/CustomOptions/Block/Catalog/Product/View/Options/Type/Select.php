...
class MageWorx_CustomOptions_Block_Catalog_Product_View_Options_Type_Select extends Mage_Catalog_Block_Product_View_Options_Type_Select {
    ...
    public function getValuesHtml() {
        ...
        if ($_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN || $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_MULTIPLE) {            
            ...
            foreach ($_option->getValues() as $_value) {
                $count++;
                //echo '<pre>';print_r($_value->getData());
                $priceStr = $helper->getFormatedOptionPrice(
                        $this->_formatPrice(array(
                            'is_percent' => ($_value->getPriceType() == 'percent') ? true : false,
                            'pricing_value' => $_value->getPrice(true)
                ...
                if($_option->getTitle()!="Color-Size")
                {
                if ($disabled && $enabledDependent && $helper->hideDependentOption() && $_option->getIsDependent()) $selectHtml .= '<li style="display: none;">'; else $selectHtml .= '<li>';
                 $selectHtml .= $helper->getImgHtml($_value->getImagePath(), $_option->getId());
                    ...
                    //$val_title=$_value->getTitle();
                    $op_val=explode("-",$_value->getTitle());
                    //echo $val_title[1];
                    if($count==2 )
                    { 
                        $selectHtml .= '<tr class="attributes-value-header"><td></td>';
                        $count1=1;
                        //echo $total_options;
                        //echo '<pre>';print_r($_option->getData());
                        foreach ($_option->getValues() as $_val) {
                         //echo (int)sqrt($total_options);
                           $total_Sqrt=(int)sqrt($total_options);
                            if($count1<=$total_Sqrt)
                            {
                             $op_val2=explode("-",$_val->getTitle());
                              //echo '<pre>';print_r($op_val2);
                             $selectHtml .= '<td><p>'.$op_val2[0].'</p></td>';
                             $count1++;
                            }
                        }
                        $selectHtml .= '</tr>';
                    }
                    if($op_val[1]!=$val_title[1])
                    {
                       // $val_title=$_value->getTitle();
                        $selectHtml .= '<tr>';
                    }
                    $selectHtml .= $helper->getImgHtml2($op_val[1],$_value->getImagePath(), $_option->getId());
                }
                ...
                if ($helper->isQntyInputEnabled() && $_option->getQntyInput() && $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_CHECKBOX) {                    
                    ...
                    if (!$optionValueQty) $optionValueQty = '';
                    if($_option->getTitle()=="Color-Size"){$selectHtml .='<td>';}
                    if($_option->getTitle()!="Color-Size"){
                    $selectHtml .=
                      ...
                      //  $checked="checked";
                        $selectHtml .=
                        '<input  ' . $checked . ' style="display:none" type="' . $type . '" class="' . $class . ' ' . $require . ' product-custom-option" onclick="$(\'options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty\').disabled=!this.checked; if ($(\'options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty\').value<=0) $(\'options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty\').value=1; '.(($enabledDependent)?'dependentOptions.select(this);':'').((Mage::app()->getStore()->isAdmin())?'':'opConfig.reloadPrice();').$setProdutQtyFunc.'" name="options[' . $_option->getId() . ']' . $arraySign . '" id="options_' . $_option->getId() . '_' . $count . '" value="' . $_value->getOptionTypeId() . '" />';
                    }
                           if($_option->getTitle()!="Color-Size"){
                               $selectHtml .='<label for="options_' . $_option->getId() . '_' . $count . '">' . $_value->getTitle() . ' ' . $priceStr . $qty . '</label>
                            &nbsp;&nbsp;&nbsp;';
                           }
                           if($_option->getTitle()!="Color-Size"){
                               $selectHtml .='<label><b>'.$helper->getDefaultOptionQtyLabel().'</b>';
                               $selectHtml .='<input type="text" class="input-text qty validate-greater-than-zero" value="'.$optionValueQty.'" maxlength="12" id="options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty" name="options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty" onchange="'.((Mage::app()->getStore()->isAdmin())?'':'opConfig.reloadPrice();').'" onKeyPress="if(event.keyCode==13){'.((Mage::app()->getStore()->isAdmin())?'':'opConfig.reloadPrice();').'}" '.($checked?$disabled:'disabled').'></label>
                         </span>';
                           }
                           else
                           {
                               $selectHtml .='<label>';
                               //='options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty';
                               $selectHtml .='<input type="text"  class="input-text qty" value="'.$optionValueQty.'" maxlength="12" id="options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty" name="options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty" onchange="optionsSetQtyProduct.setQty1($(this)); if($(this).value>0){$(\'options_' . $_option->getId() . '_' . $count . '\').checked=\'checked\'}else{$(\'options_' . $_option->getId() . '_' . $count . '\').checked=\'\'}"  ></label>';
                           }
                ...
                $selectHtml .= '</li>';
                    ...
                    $val_title=explode("-",$_value->getTitle());//$_value->getTitle();
                    if($op_val[1]!=$val_title[1])
                    {
                        $selectHtml .= '</tr>';
                    }
            ...
            if ($selectHtml) $selectHtml = '<ul id="options-' . $_option->getId() . '-list" class="options-list">'.$selectHtml.'</ul>';
                ...
                if ($selectHtml) $selectHtml = '<table id="bulk-table" class="options-list">'.$selectHtml.'</table>';
            }
